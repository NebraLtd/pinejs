...
const fixturePath = __dirname + '/fixtures/04-metadata/config';
...
describe('04 metadata', function () {
	...
	// TODO check different method permissions on endpoints
	// TODO check 1-m-n-1  relationship resource
	...
	describe('get metadata', () => {
		it('check /example/$metadata is served by pinejs', async () => {
			...
			console.log(`res.body.paths:${JSON.stringify(res.body.paths, null, 2)}`);
			const gatewayConnectsDevice =
				res.body.paths['/gateway__connects__device'];
			expect(gatewayConnectsDevice).to.have.keys(['get', 'post']);
